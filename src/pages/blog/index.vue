...
import LoadingProductCard from '~/components/ui/LoadingProductCard.vue'
  ...
  >
    ...
    <div v-else class="grid grid-cols-3 gap-6 pb-10">
      <div class="col-span-3">
        <div
          class="flex flex-col items-center rounded-2xl bg-neutral-50 md:flex-row shadow-xl"
        >
          <div class="w-full md:w-1/2">
            <img
              data-aos="fade-right"
              class="h-full max-h-64 w-full rounded-l-lg object-cover sm:max-h-96"
              :src="asImageSrc(blogStore.featuredBlog.image)"
            >
          </div>
          <div
            class="mb-6 flex h-full w-full flex-col items-start justify-center py-6 md:mb-0 md:w-1/2"
          >
            <div
              class="flex h-full transform flex-col items-start justify-center space-y-3 md:space-y-5 md:pl-10 lg:pl-16"
            >
              <div
                class="bg-primary-500 flex items-center pl-2 pr-3 py-1.5 leading-none rounded-full text-xs font-medium uppercase text-white inline-block"
              >
                <svg
              ...
              <h2
                class="text-4xl font-bold leading-none lg:text-3xl xl:text-5xl"
              >
                <p>{{ blogStore.featuredBlog.title }}</p>
                <p class="pt-3 pr-6 text-neutral-500 text-lg line-clamp-3">
                  {{ asText(blogStore.featuredBlog.content) }}
                </p>

                <p class="pt-2 text-lg font-medium">
                  Posted by
                  <span class="mr-1 underline">{{
                    blogStore.featuredBlog.author
                  }}</span>.
                </p>
              </h2>
