...
<template>
  <div class="mx-auto flex flex-wrap  max-w-7xl">
    <div
      class="mb-10 w-4/5 items-center border-neutral-200 pb-10 md:mb-0 md:border-r-2 px-8 md:border-b-1"
    >
      <img class="pb-4 w-full h-2/3" :src="asImageSrc(blog.image)">
      <span
        class="text-primary-500 text-sm border-primary-500"
      >Category : {{ asText(blog.title) }}</span>
      <h1 class="prose lg:prose-xl hover:text-primary-500  text-upper">
        {{ asText(blog.title) }}
      </h1>
      <p class="text-base pb-4 leading-relaxed">
        {{ asText(blog.content) }}
      </p>
      <hr>
    ...
    <!-- side page -->
    <div class="mx-auto  flex-wrap px-5 w-1/5">
      <SideBlogCard
        v-for="blog in blogStore.detailBlog"
        :key="blog.uid"
        :title="blog.title"
        :date="blog.date"
